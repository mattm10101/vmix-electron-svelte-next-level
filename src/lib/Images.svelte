<script>
  ...
  // UPDATED: Now targets the images list input
  const IMAGES_INPUT_NAME = 'LIST - IMAGES'

  // Reactive state derived from stores
  $: imagesInput = $inputs.find((i) => i.shortTitle === IMAGES_INPUT_NAME)
  $: isPlaying = imagesInput && $playingInputs.has(imagesInput.id)
  // Mute logic is removed
  $: trackList = imagesInput?.list || []
  $: vmixSelectedIndex = imagesInput?.selectedIndex
    ? imagesInput.selectedIndex - 1
    : -1
  $: nowPlayingName = (() => {
    if (!imagesInput || !trackList[vmixSelectedIndex]) return 'Paused'
  ...
  async function refreshSelectedIndex() {
    ...
    const xpath = `vmix/inputs/input[@shortTitle='${IMAGES_INPUT_NAME}']/@selectedIndex`
    const newIndex = await queryVmixXpath(xpath)

    if (newIndex !== null) {
      inputs.update((allInputs) => {
        const targetInput = allInputs.find((i) => i.id === imagesInput.id)
        if (targetInput) {
  ...
  function selectTrack(trackIndex) {
    const inputName = imagesInput.shortTitle
  ...
  async function handleFlashClick(command) {
    sendCommand(`FUNCTION ${command} Input=${imagesInput.shortTitle}`)
...
<div class="images-container">
  {#if imagesInput}
    ...
    <div class="button-row">
      ...
      <button
        ...
        on:click={() =>
          sendCommand(`FUNCTION PlayPause Input=${imagesInput.shortTitle}`)}
        title="Play/Pause"
  ...
  {:else}
    <div class="placeholder">Input "LIST - IMAGES" not found.</div>
  {/if}
</div>
